...
### 4. Create and set the shared secret
...
```bash
npx wrangler secret put OLLAMA_SECRET   # paste the value
```
...
For local testing of the auth layer, create a `.dev.vars` from the example:

```bash
cp .dev.vars.example .dev.vars   # then set OLLAMA_SECRET
```
...
### Centralizing the secret with Secrets Store

AI Gateway BYOK is backed by [Cloudflare Secrets Store](https://developers.cloudflare.com/secrets-store/). For a single source of truth, bind the Worker's `OLLAMA_SECRET` to a [Secrets Store secret](https://developers.cloudflare.com/secrets-store/integrations/workers/) and reference the same secret for BYOK (via the API naming convention `{gateway_id}_{provider_slug}_{alias}`). Then one rotation in Secrets Store updates both sides at once.
